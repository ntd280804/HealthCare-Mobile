...
namespace WebApp.Areas.Public.Controllers
{
    ...
    public class SessionController : Controller
    {
        ...
        {
            if (!_oracleClientHelper.TrySetHeaders(_httpClient, out var redirect, isAdmin: false))
                return redirect;

            var model = new SessionPageViewModel
            {
                CurrentPlatform = HttpContext.Session.GetString("CPlatform") ?? string.Empty,
                CurrentSessionId = HttpContext.Session.GetString("CSessionId") ?? string.Empty
            };
