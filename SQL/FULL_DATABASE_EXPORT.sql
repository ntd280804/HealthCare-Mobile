...
------SQL dùng để sửa lỗi đăng ký Khách hàng-----
CREATE OR REPLACE PROCEDURE APP.REGISTER_CUSTOMER (
    p_phone     IN VARCHAR2,
    p_full_name IN VARCHAR2,
    p_password  IN VARCHAR2,
    p_email     IN VARCHAR2,
...
BEGIN
    v_hash := HASH_PASSWORD(p_password);
    v_keys := CRYPTO.RSA_GENERATE_KEYS(KEY_SIZE => 1024);
    v_pub := TRIM(SUBSTR(v_keys,
              INSTR(v_keys,'publicKey start*****') + LENGTH('publicKey start*****'),
              INSTR(v_keys,'****publicKey end') - (INSTR(v_keys,'publicKey start*****') + LENGTH('publicKey start*****'))));
    v_priv := TRIM(SUBSTR(v_keys,
              INSTR(v_keys,'privateKey start****') + LENGTH('privateKey start****'),
              INSTR(v_keys,'****privateKey end') - (INSTR(v_keys,'privateKey start****') + LENGTH('privateKey start****'))));
    v_pwd := HASH_PASSWORD_20CHARS(p_password);

    BEGIN
        EXECUTE IMMEDIATE 'CREATE USER "' || p_phone || '" IDENTIFIED BY "' || v_pwd || '" DEFAULT TABLESPACE USERS QUOTA UNLIMITED ON USERS';
        EXECUTE IMMEDIATE 'GRANT ROLE_KHACHHANG TO "' || p_phone || '"';
        EXECUTE IMMEDIATE 'ALTER USER "' || p_phone || '" GRANT CONNECT THROUGH APP'; -- fixed quotes
    EXCEPTION
        WHEN OTHERS THEN
            RAISE_APPLICATION_ERROR(-20002, 'Không thể tạo DB User cho Customer ' || p_phone || ': ' || SQLERRM);
    END;
    ...
    EXCEPTION
        WHEN OTHERS THEN
            ROLLBACK;
            BEGIN EXECUTE IMMEDIATE 'DROP USER "' || p_phone || '" CASCADE'; EXCEPTION WHEN OTHERS THEN NULL; END;
            RAISE;
    END;
END;
/
