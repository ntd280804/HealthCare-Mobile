...
class EmployeeImportDetailScreen extends StatefulWidget {
  final int stockInId;

  const EmployeeImportDetailScreen({super.key, required this.stockInId});

  @override
  State<EmployeeImportDetailScreen> createState() => _EmployeeImportDetailScreenState();
}

class _EmployeeImportDetailScreenState extends State<EmployeeImportDetailScreen> {
  ...
  Future<void> _loadDetails() async {
    ...
    try {
      final details = await _api.getImportDetails(widget.stockInId);
      setState(() {
        _details = details;
        _isLoading = false;
      });
  ...
  Future<void> _downloadPdf() async {
    ...
    try {
      final pdfBytes = await _api.downloadImportPdf(widget.stockInId);
      
      final directory = await getApplicationDocumentsDirectory();
      final file = File('${directory.path}/ImportInvoice_${widget.stockInId}.pdf');
  ...
  Future<void> _verifySign() async {
    ...
    try {
      final result = await _api.verifyImportSign(widget.stockInId);
      final isValid = result['isValid'] == true || result['isValid'] == 1;
      final message = result['message'] ?? result['Message'] ?? '';
      final stockInId = result['StockInId'] ?? result['stockInId'] ?? widget.stockInId;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Chi tiết hóa đơn nhập #${widget.stockInId}'),
      ),
      body: _isLoading
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              _buildInfoRow('ID', _details!['stockInId']?.toString() ?? ''),
                              _buildInfoRow('Nhân viên', _details!['empUsername']?.toString() ?? ''),
                              _buildInfoRow('Ngày', _formatDate(_details!['inDate'])),
